import {IFactory, makeFactoryFrom} from "../src";
...
describe('Factory', () => {
  ...
  it('withProperties should set properties from args', () => {
    const expected = { ...testingProductWithEmptyArray, id: '1', title: 'test' };
  ...
  describe('randomizeParams() should randomize property from args', () => {
    ...
    let testingFactory: IFactory<any>;

    beforeEach(() => (testingFactory = makeFactoryFrom(testingProductWithEmptyArray)))
    ...
    test('Array length 0', () => {
      const arrayResult = () => testingFactory.randomizeParams( 'products').build();
